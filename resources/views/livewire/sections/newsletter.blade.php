<div>
    <div class="w-[92%] mx-auto pt-5 mb-20">
        <!-- Newsletter Title -->
        <div class="flex flex-col items-center">
            <div class="md:w-[800px] text-wrap">
                <h1 class="mb-4 text-3xl font-bold text-center md:text-5xl ">
                    JOIN OUR NEWSLETTER
                </h1>
                <p class="mb-6 leading-loose text-center text-wrap ">Capturing life's beauty through my lens. Specializing in wedding, portrait, and commercial photography. Contact me to bring your vision to life..</p>
            </div>
            <!-- Newsletter Promopt -->
            <div>
                <input class="p-1 px-12 text-xl text-left border rounded-md pl-[14px]" type="text" name="" placeholder="your email">
                <button class="p-2 text-white bg-black rounded-md md:px-8">subscribe</button>
            </div>
        </div>
    </div>
</div>
